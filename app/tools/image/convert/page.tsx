...
const formatOptions = [
  { value: "jpeg" as ImageFormat, label: "JPG", mimeType: "image/jpeg" },
  { value: "png" as ImageFormat, label: "PNG", mimeType: "image/png" },
  { value: "webp" as ImageFormat, label: "WEBP", mimeType: "image/webp" },
];
  ...
  const convertImage = async () => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        const targetMimeType = formatOptions.find(
          (opt) => opt.value === targetFormat
        )?.mimeType;
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <nav className="mb-8 text-sm">
        ...
        <span className="mx-2 text-text-muted">/</span>
        <span className="text-secondary font-medium">格式转换</span>
      </nav>
              ...
              <div className="flex gap-4">
                {formatOptions.map((option) => (
                  <button
                    ...
                    className={`flex-1 px-6 py-4 rounded-xl font-semibold text-lg transition-all duration-200 border-2 ${
                      targetFormat === option.value
                        ? "bg-primary border-primary text-white"
                        : "bg-white border-gray-200 text-secondary hover:border-primary-light"
                    }`}
                  >
                    {option.label}
                  </button>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-green-700 text-sm">
                      已转换为 {targetFormat.toUpperCase()} 格式
                    </p>
                  </div>
                  <button
                    type="button"
